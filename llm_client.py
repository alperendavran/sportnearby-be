...
class OllamaClient:
    ...
    def __init__(self):
        ...
        if settings.langsmith_tracing:
            ...
            print(f"✅ LangSmith tracing aktif: {settings.langsmith_project}")
        else:
            self.langsmith_client = None
            print("⚠️ LangSmith tracing kapalı")
    ...
    async def classify_intent(self, text: str) -> IntentDecision:
        system = (
          ...
          "- 'events_in_cities': User wants events in specific cities (e.g., 'Brussels spor etkinlikleri', 'Antwerp maçları')\n"
          "- 'events_by_competition': User wants events for specific competitions (e.g., 'Pro League maçları', 'Jupiler League')\n"
          "- 'events_by_venue': User wants events at specific venues (e.g., 'Lotto Park maçları')\n"
          "- 'next_at_venue': User wants next event at a specific venue (e.g., 'Lotto Park'ta sıradaki maç')\n"
          "- 'venues_near': User wants venues near their location (e.g., 'yakınımdaki stadyumlar')\n"
          "- 'general_inquiry': General questions about sports (e.g., 'spor aktiviteleri nerede yapabilirim', 'ne yapabilirim')\n"
    ...
    async def resolve_date_range(self, text: str, now_dt: Optional[datetime]=None) -> DateRangeOut:
        ...
        system = (
            ...
            "  'time_keyword':'today'|'tonight'|'tomorrow'|'this_weekend'|'next_weekend'|'this_week'|'next_week'|'weeks_ahead'|'next_year'|null,"
            ...
            "- 'weeks_ahead' = multiple weeks from CURRENT_DATE (e.g., '8 hafta içerisinde' = 8 weeks from today).\n"
            "- 'next_year' = next calendar year (e.g., 'gelecek yıl' = January 1 to December 31 of next year).\n"
            "- 'tonight' = CURRENT_DATE (date level; do not output time).\n"
            "- For 'X hafta içerisinde' or 'X weeks within': date_from = CURRENT_DATE, date_to = CURRENT_DATE + X weeks.\n"
            "- For 'gelecek yıl' or 'next year': date_from = January 1 of next year, date_to = December 31 of next year.\n"
            "  Example: If CURRENT_DATE is 2025-09-26, then 'gelecek yıl' = date_from:'2026-01-01', date_to:'2026-12-31'.\n"
            ...
            "- If time-related words are mentioned but ambiguous, return status='UNCLEAR' and null dates.\n"
        ...
        user = (
            ...
            f"TEXT: {text}\n"
            "Examples:\n"
            "- 'gelecek yıl' or 'next year' → status:'OK', time_keyword:'next_year', date_from:'2026-01-01', date_to:'2026-12-31'\n"
            "- 'bu hafta sonu' → status:'OK', time_keyword:'this_weekend', date_from:'2025-09-27', date_to:'2025-09-28'\n"
            "- 'spor etkinlikleri' → status:'NO_TIME', time_keyword:null, date_from:null, date_to:null\n"
            "Return ONLY JSON."
        )
        ...
        if out.status == "OK" and out.date_from and out.date_to:
            try:
                ...
                # 2) yıl sapması (±500 gün sınırı - gelecek yıl için yeterli)
